...
impl<R> Prefix<R> {
    fn prefix_key(&self, key: KeyRef) -> KeyRef {
        key.into_owned()
...
mod tests {
    ...
    fn test_basic_functionality() {
        ...
        layered.register_counter(Key::from_name("counter_metric").into(), None);
        layered.register_gauge(Key::from_name("gauge_metric").into(), None);
        layered.register_histogram(Key::from_name("histogram_metric").into(), None);
