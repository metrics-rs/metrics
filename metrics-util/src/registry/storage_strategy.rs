...
use super::{AtomicStorage, Storage};
...
/// Determines how keys are preprocessed before they are inserted into a registry.
pub trait StorageStrategy<K> {
    /// Preprocesses a key for insertion.
    fn preprocess(key: &K) -> K;
}

/// Clones keys before insertion.
#[derive(Debug)]
pub struct CloneOnInsert;

impl<K: Clone> StorageStrategy<K> for CloneOnInsert {
    #[inline]
    fn preprocess(key: &K) -> K {
        key.clone()
    }
...
/// Normalizes [`Key`] values into a retained form before insertion.
#[derive(Debug)]
pub struct RetainOnInsert;

impl StorageStrategy<Key> for RetainOnInsert {
    #[inline]
    fn preprocess(key: &Key) -> Key {
        key.to_retained()
    }
}

/// A high-performance metric registry with configurable key storage strategy.
///
...
///
/// In some cases, users may prefer `GenerationalAtomicStorage` when knowing if a metric has been
...
#[derive(Debug)]
pub struct Registry<K, S, Strategy = CloneOnInsert>
where
    S: Storage<K>,
...
impl<Strategy> Registry<Key, AtomicStorage, Strategy>
where
    Strategy: StorageStrategy<Key>,
{
    /// Creates a new `Registry` using a regular [`Key`] and atomic storage.
    pub fn atomic() -> Self {
        Self::new(AtomicStorage)
    }
}

impl<K, S, Strategy> Registry<K, S, Strategy>
where
    S: Storage<K>,
    Strategy: StorageStrategy<K>,
{
    ...
    pub fn new(storage: S) -> Self {
        ...
        Self { counters, gauges, histograms, shard_mask, storage, _strategy: PhantomData }
    }
        ...
        O: FnOnce(&S::Counter) -> V,
    {
        ...
        if let Some((_, v)) = shard_read.raw_entry().from_key_hashed_nocheck(hash, key) {
            ...
            let v = if let Some((_, v)) = shard_write.raw_entry().from_key_hashed_nocheck(hash, key)
            {
                ...
                let key = Strategy::preprocess(key);
                let (_, v) = shard_write
                    .raw_entry_mut()
                    .from_key_hashed_nocheck(hash, &key)
                    .or_insert_with(|| {
                        let value = self.storage.counter(&key);
                        (key, value)
                    });
        ...
        if let Some((_, v)) = shard_read.raw_entry().from_key_hashed_nocheck(hash, key) {
            ...
            let v = if let Some((_, v)) = shard_write.raw_entry().from_key_hashed_nocheck(hash, key)
            {
                ...
                let (_, v) = shard_write
                    ...
                    .from_key_hashed_nocheck(hash, &key)
                    .or_insert_with(|| {
                        let value = self.storage.gauge(&key);
                        (key, value)
                    });
        ...
        if let Some((_, v)) = shard_read.raw_entry().from_key_hashed_nocheck(hash, key) {
            ...
            let v = if let Some((_, v)) = shard_write.raw_entry().from_key_hashed_nocheck(hash, key)
            {
                ...
                let (_, v) = shard_write
                    ...
                    .from_key_hashed_nocheck(hash, &key)
                    .or_insert_with(|| {
                        let value = self.storage.histogram(&key);
                        (key, value)
                    });
