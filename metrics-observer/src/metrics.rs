...
use proto::event::Event as ProtoEvent;
...
// Intermediate types used to satisfy clippy's dislike of complexity.
type MetadataKey = (MetricKind, String);
type MetdataValue =  (Option<Unit>, Option<String>);
type MetadataStorage = Arc<RwLock<HashMap<MetadataKey, MetdataValue>>>;
pub struct Client {
            ...
            let next = match self.state {
                ...
                RunnerState::Connected(ref mut stream) => {
                    ...
                    loop {
                        ...
                        let message = match self::proto::Event::decode_length_delimited(&mut buf) {
                            Err(e) => {
                        ...
                        match event {
                            ...
                            ProtoEvent::Metric(metric) => {
                                ...
                                match metric.operation.expect("no metric operation") {
                                    proto::metric::Operation::IncrementCounter(value) => {
                                    ...
                                    proto::metric::Operation::SetCounter(value) => {
                                        let key = CompositeKey::new(MetricKind::Counter, key_data);
                                        let mut metrics = self.metrics.write().unwrap();
                                        let counter = metrics
                                            .entry(key)
                                    ...
                                    proto::metric::Operation::IncrementGauge(value) => {
                                    ...
                                    proto::metric::Operation::DecrementGauge(value) => {
                                        let key = CompositeKey::new(MetricKind::Gauge, key_data);
                                        let mut metrics = self.metrics.write().unwrap();
                                        let gauge = metrics
                                            .entry(key)
                                    ...
                                    proto::metric::Operation::SetGauge(value) => {
                                        let key = CompositeKey::new(MetricKind::Gauge, key_data);
                                        let mut metrics = self.metrics.write().unwrap();
                                        let gauge = metrics
                                            .entry(key)
                                    ...
                                    proto::metric::Operation::RecordHistogram(value) => {
                                        let key =
