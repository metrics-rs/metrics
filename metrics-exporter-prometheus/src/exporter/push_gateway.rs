    ...
    Box::pin(async move {
        #[cfg(target_arch = "wasm32")]
        {
            ...
            loop {
                ...
                let mut req = client
                    .request(http_method.clone(), endpoint.to_string())
                    .header("Content-Type", "text/plain");

                if let Some(username) = &username {
                    let password = password.as_deref();
                    let auth_header_val = basic_auth(username, password);
                    req = req.header("Authorization", auth_header_val);
                }

                match req.body(metrics).send().await {
                    Ok(response) => {
                        ...
                        if !status.is_success() {
                            ...
                            error!(
                                "unexpected status after pushing metrics to push gateway: {}, body: {}",
                                status, body
                            );
                        }
                    }
